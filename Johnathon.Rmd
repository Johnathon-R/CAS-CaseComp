...
policy_tbl <- model_cov2 %>%
  ...
  summarise(
    pure_total = sum(pure_cov, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
...
Allocate policy premiums back to coverages proportionally
```{r}

priced_cov <- model_cov2 %>%
  left_join(policy_tbl, by = c("student_id", "risk_tier", "holdout")) %>%
  group_by(student_id) %>%
  mutate(
    # allocation weight: share of pure premium
    w = ifelse(pure_total > 0, pure_cov_cap / pure_total, 1/4),
    premium_cov = premium_total * w
  ) %>%
  ungroup()
```
...
final_coverage_prices <- priced_cov %>%
  select(
    ...
    base_pure_cov, relativity_cov,
    pure_cov_cap, premium_cov
  ) %>%
