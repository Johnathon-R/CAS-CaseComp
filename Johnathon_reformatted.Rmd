...
# Filter the x-th percentile of liability losses from our model. 
# choose underwriting percentiles by coverage
uw_tbl <- tibble::tribble(
  ~coverage, ~q,
  "Liability", 0.999, #(10 people)
  "Guest Medical", 0.999, 
  "Additional Living Expense", 0.999,
  "Personal Property", 0.999
)

#(0.999, 0.995, 0.995, 0.995) give error of 5, 18, 8
#(1, 0.997, 0.995, 0.995) give error of 2, 11, 16
#(0.999, 0.999, 0.999, 0.999) give error of 0, 3, 25 - now just put cap on price

# 2. Perform the calculation in one flow
model_cov2 <- model_cov2 %>%
  select(-any_of(c("q", "cut", "uw_exclude"))) %>%   # Remove old columns if they exist to prevent .x/.y suffixes
  left_join(uw_tbl, by = "coverage") %>% # Bring in the 'q' values for each coverage
  group_by(coverage) %>%   # Group by coverage so quantile is calculated per category
  mutate(cut = quantile(loss, unique(q), na.rm = TRUE)) %>%  # Filter immediately (only keep rows <= the cut)
  filter(loss <= cut) %>% # Cleanup: Remove the temporary helper columns
  select(-q, -cut) %>%
  ungroup()
...
Run this to see how many people we are excluding from each category.
```{r}
model_cov2 %>%
  group_by(coverage) %>%
  summarise(
    target_q = max(q), 
    max_allowed = max(cut), 
    excluded_count = sum(uw_exclude, na.rm = TRUE)
  )
```
...
```{r}
# 1) Prep variables (do this once)
tw_fit_interactions <- glm(
  loss ~ 
    # Main effects
    risk_tier + coverage + gender + greek + class + study +
    
    # Tier-specific effects for key risk factors
    risk_tier:(off_campus + log_dist + sprinklered) +
    
    # Coverage-specific effects (as before)
    coverage:(off_campus + log_dist + sprinklered),
...
model_cov2$pure_cov <- predict(tw_fit, 
                              newdata = model_cov2, 
                              type = "response")
```
...
Note:\
Changed the quantile cap for liabilities since there are not enough data points thus making 99.5th quantile way to aggresive. For instance, we would have only 2 data point in tier 2 liabilities category.
```{r}
# The 99.5th quantile of prices to "Cap" rates at this value.
cap_tbl <- model_cov2 %>%
  filter(!holdout) %>%
  group_by(coverage) %>%
  summarise(cap = quantile(pure_cov, 0.95), .groups="drop")

model_cov2 <- model_cov2 %>%
  group_by(coverage) %>%
  mutate(
    cap = quantile(pure_cov[!holdout], 0.95),
    pure_cov_cap = pmin(pure_cov, cap)
  ) %>%
  ungroup()
```
...
cat("\n=== MODEL COMPARISON ===\n")
cat("Original model AIC:", AIC(tw_fit), "\n")
cat("Interaction model AIC:", AIC(tw_fit_interactions), "\n")
cat("Improvement:", AIC(tw_fit) - AIC(tw_fit_interactions), "\n")

# Predict with new model
model_cov2$pure_cov_interactions <- predict(tw_fit_interactions, 
                                             newdata = model_cov2, 
                                             type = "response")

# Re-calculate capped values with new predictions
model_cov2 <- model_cov2 %>%
  mutate(pure_cov_cap_interactions = pmin(pure_cov_interactions, cap))

# Update base rates and relativities
base_by_tier_cov_interactions <- model_cov2 %>%
  filter(!holdout) %>%
  group_by(risk_tier, coverage) %>%
  summarise(base_pure_cov_interactions = mean(pure_cov_interactions), 
            .groups = "drop")

model_cov2 <- model_cov2 %>%
  left_join(base_by_tier_cov_interactions, by = c("risk_tier", "coverage")) %>%
  mutate(relativity_cov_interactions = pure_cov_cap_interactions / base_pure_cov_interactions)
...
train %>%
  ...
  summarise(
    ...
    n_pos = sum(pos),
    pos_rate = mean(pos),
    mean_pos = ifelse(sum(pos) > 0, mean(loss[pos]), NA_real_),
    .groups = "drop"
  )

train_scored <- train %>%
  mutate(pred = predict(tw_fit, newdata = train, type = "response"))
...
calibration <- c(
  ...
  pred_mean   = mean(hold$pure_cov_cap),
  error_pct = abs(mean(holdout_data$loss) - mean(holdout_data$pure_cov_cap)) / mean(holdout_data$loss) * 100
)
print(calibration)

decile_results <- hold %>%
  mutate(decile = dplyr::ntile(pure_cov_cap, 10)) %>%
  group_by(decile) %>%
  summarise(actual = mean(loss), pred = mean(pure_cov_cap), n = n(), .groups="drop")
...
#  filter(!holdout) %>%
#  group_by(risk_tier, coverage) %>%
#  summarise(base_pure_cov = mean(pure_cov_cap), .groups = "drop")


K <- 50  # tuning constant; larger => more pooling toward overall coverage mean

tier_cov_stats <- model_cov2 %>%
  filter(!holdout) %>%
  group_by(risk_tier, coverage) %>%
  summarise(
    n_pos = sum(loss > 0),
    base_raw = mean(pure_cov_cap),
    .groups = "drop"
  )

cov_stats <- model_cov2 %>%
  filter(!holdout) %>%
  group_by(coverage) %>%
  summarise(
    base_cov = mean(pure_cov_cap),
    .groups = "drop"
  )

base_by_tier_cov <- tier_cov_stats %>%
  left_join(cov_stats, by = "coverage") %>%
  mutate(
    Z = n_pos / (n_pos + K),
    base_pure_cov = Z * base_raw + (1 - Z) * base_cov
  ) %>%
  select(risk_tier, coverage, base_pure_cov)
...
error_metrics <- holdout_data %>%
  summarise(
    ...
    predicted_mean = mean(pure_cov_cap),
    mean_error = predicted_mean - actual_mean,
    mean_abs_error = mean(abs(pure_cov_cap - loss)),
    mean_squared_error = mean((pure_cov_cap - loss)^2),
    root_mean_squared_error = sqrt(mean((pure_cov_cap - loss)^2)),
    
    # Percentage errors
    mean_abs_percentage_error = mean(abs((pure_cov_cap - loss) / pmax(loss, 1))) * 100,
    mean_percentage_error = mean((pure_cov_cap - loss) / pmax(loss, 1)) * 100,
    
    # Correlation
    correlation = cor(pure_cov_cap, loss, use = "complete.obs"),
...
error_by_coverage <- holdout_data %>%
  ...
  summarise(
    actual = mean(loss),
    predicted = mean(pure_cov_cap),
    bias = predicted - actual,
    abs_error = mean(abs(pure_cov_cap - loss)),
    mse = mean((pure_cov_cap - loss)^2),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
...
comprehensive_metrics <- holdout_data %>%
  group_by(risk_tier, coverage) %>%
  summarise(actual = mean(loss),
            pred = mean(pure_cov_cap),
            pct_err = (mean(pure_cov_cap) - mean(loss))/mean(loss) * 100,
            n = n(),
            .groups="drop")

print(comprehensive_metrics)
```
# ISSUE IS TIER 2 LIABILITY

Goal: remove tier 2 liability outliers that are skewing our model
```{r}
liab <- model_cov2 %>%
  filter(coverage == "Liability") %>%
  mutate(
    loss_pos = loss > 0
  )

liab_train <- liab %>% filter(!holdout)

liab_train <- liab_train %>%
  mutate(pred_liab = pure_cov_cap)   # use your chosen liability prediction field

cut <- quantile(liab_train %>% filter(risk_tier == 2) %>% pull(loss), 0.995, na.rm=TRUE)

exclude_rule <- liab %>%
  mutate(exclude = (risk_tier == 2) & (pure_cov_cap >= cut))
